...
def main(session: snowpark.Session):
    ...
    # Load Actual data
    actual_df = session.table("FCD_090_DB_DEV.PROJECT_CASHFLOW_DATA.ACTUALVALUE").select(
        col("PROJECT_NUMBER"),
        col("STAGE"),
        col("SUB_STAGE"),
    ...
    # Load Planned data
    planned_df = session.table("FCD_090_DB_DEV.PROJECT_CASHFLOW_DATA.PLANNEDVALUE").select(
        col("PROJECT_NUMBER"),
        col("STAGE"),
        col("SUB_STAGE"),
        col("DATE"),
    ...
    # Load Forecast data
    forecast_df = session.table("FCD_090_DB_DEV.PROJECT_CASHFLOW_DATA.FORECASTVALUE").select(
        col("PROJECT_NUMBER"),
        col("STAGE"),
        col("SUB_STAGE"),
    ...
    combined_snowpark_df.write.mode("overwrite").save_as_table("FCD_090_DB_DEV.PROJECT_CASHFLOW_DATA.COMBINED_CURVES")
