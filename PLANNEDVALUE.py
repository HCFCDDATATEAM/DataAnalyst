...
from snowflake.snowpark.functions import col, sum as sf_sum, lit
import pandas as pd
...
def load_actuals_with_invoice_date(session: snowpark.Session):
    ...
    def select_invoice():
        return session.table("LANDING_DB.FCD_UNIFIER.UNIFIER__Z_BP_INVOICE").select(
            ...
            col("INVOICELINEAMOUNT").alias("ACTUALS_COST")
        )
...
def main(session: snowpark.Session):
    ...
    output_df.write.mode("overwrite").save_as_table("FCD_090_DB_DEV.PROJECT_CASHFLOW_DATA.PLANNEDVALUE")
    return output_df
